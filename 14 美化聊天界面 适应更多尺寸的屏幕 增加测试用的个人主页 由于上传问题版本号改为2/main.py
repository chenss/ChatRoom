...
class BaseRuquestHandler(webapp2.RequestHandler):
...
class MainRequestHandler(BaseRuquestHandler):
    ...
    def get(self):
        ...
        template_values = self.getBaseInfo()
...
class UserHomePageHandler(BaseRuquestHandler):
    def get(self):
        
        template_values = self.getBaseInfo()
        
        dir = os.path.dirname(__file__)
        path = os.path.join(dir, 'home.html')
        self.response.write(template.render(path, template_values))
