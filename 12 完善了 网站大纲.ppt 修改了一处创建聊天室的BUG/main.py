...
class MainRequestHandler(webapp2.RequestHandler):
    ...
    def getRooms(self):
        room_query = db.GqlQuery("SELECT * FROM Rooms ORDER BY created_time DESC LIMIT 20")
        rooms = room_query
...
class CreateRoomHandler(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        query = Rooms.all().filter('name ==', name)
        entity = query.get()
        if entity or (name == None):
            raise Exception('name must have a unique value and not None!')
        else:
            introduce = self.request.get('introduce')
            room = Rooms()
            room.name = name
            room.intro = introduce
            room.put()
